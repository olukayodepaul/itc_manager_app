...
class AddCustomerViewModel : ViewModel() {
    ...
    private fun contactPerson(contactPerson: String) =
        updateState { copy(contactPerson = contactPerson) }

    private fun mobileNumber(mobileNumber: String) =
        updateState { copy(mobileNumber = mobileNumber) }
    ...
    private fun onClickConfirmButton() {
        ...
        if (state.outletName.isBlank() ||
            ...
            _addCustomerState.value =
                _addCustomerState.value.copy(errorMessage = "All fields are required!")
            return
        }
        _addCustomerState.value =
            _addCustomerState.value.copy(errorMessage = "", dialogLoader = true)
    }
